...
var _ = ginkgo.Describe("OVN cluster-manager EgressIP Operations", func() {
	...
	getEgressIPAllocatorFirstConfigSafely := func(s string) *util.ParsedNodeEgressIPConfiguration {
		fakeClusterManagerOVN.eIPC.allocator.Lock()
		defer fakeClusterManagerOVN.eIPC.allocator.Unlock()
		...
		return c.egressIPConfigs[0]
	}

	getEgressIPAllocatorSecConfigSafely := func(s string) *util.ParsedNodeEgressIPConfiguration {
		fakeClusterManagerOVN.eIPC.allocator.Lock()
		defer fakeClusterManagerOVN.eIPC.allocator.Unlock()
		c, ok := fakeClusterManagerOVN.eIPC.allocator.cache[s]
		if !ok {
		...
		return c.egressIPConfigs[1]
	}

	getEgressIPAllocatorThirdConfigSafely := func(s string) *util.ParsedNodeEgressIPConfiguration {
		fakeClusterManagerOVN.eIPC.allocator.Lock()
		defer fakeClusterManagerOVN.eIPC.allocator.Unlock()
		c, ok := fakeClusterManagerOVN.eIPC.allocator.cache[s]
		if !ok {
	...
	ginkgo.Context("WatchEgressNodes", func() {
		...
		ginkgo.It("should populated egress node data as they are tagged `egress assignable` with variants of IPv4/IPv6", func() {
			app.Action = func(ctx *cli.Context) error {
				...
				gomega.Expect(getEgressIPAllocatorFirstConfigSafely(node1.Name).V4.Net).To(gomega.Equal(node1ipV4OVNManagedNet))
				gomega.Expect(getEgressIPAllocatorFirstConfigSafely(node1.Name).V6.Net).To(gomega.Equal(node1IPv6OVNManagedNet))

				gomega.Expect(getEgressIPAllocatorSecConfigSafely(node1.Name).V4.Net).To(gomega.Equal(node1IPv4NonOVNManagedNet))
				gomega.Expect(getEgressIPAllocatorThirdConfigSafely(node1.Name).V4.Net).To(gomega.Equal(node1IPv4NonOVNManaged2Net))
				...
				gomega.Expect(getEgressIPAllocatorFirstConfigSafely(node1.Name).V4.Net).To(gomega.Equal(node1ipV4OVNManagedNet))
				gomega.Expect(getEgressIPAllocatorFirstConfigSafely(node1.Name).V6.Net).To(gomega.Equal(node1IPv6OVNManagedNet))
				gomega.Expect(getEgressIPAllocatorFirstConfigSafely(node2.Name).V4.Net).To(gomega.Equal(node2IPv4OVNManagedNet))
				gomega.Expect(getEgressIPAllocatorSecConfigSafely(node2.Name).V4.Net).To(gomega.Equal(node2IPv4NonOVNManagedNet))
				gomega.Expect(getEgressIPAllocatorThirdConfigSafely(node2.Name).V4.Net).To(gomega.Equal(node2IPv4NonOVNManaged2Net))
				return nil
			}
		...
		ginkgo.It("using retry to create egress node with forced error followed by an update", func() {
			app.Action = func(ctx *cli.Context) error {
				...
				gomega.Expect(getEgressIPAllocatorFirstConfigSafely(node.Name).V4.Net).To(gomega.Equal(ipV4Sub))
				gomega.Expect(getEgressIPAllocatorFirstConfigSafely(node.Name).V6.Net).To(gomega.Equal(ipV6Sub))
		...
		ginkgo.It("should probe nodes using grpc", func() {
			app.Action = func(ctx *cli.Context) error {
				...
				gomega.Expect(getEgressIPAllocatorFirstConfigSafely(node1.Name).V6.Net).To(gomega.Equal(ip1V6Sub))
				...
				gomega.Expect(getEgressIPAllocatorFirstConfigSafely(node1.Name).V6.Net).To(gomega.Equal(ip1V6Sub))
				gomega.Expect(getEgressIPAllocatorFirstConfigSafely(node2.Name).V4.Net).To(gomega.Equal(ip2V4Sub))
	...
	ginkgo.Context("WatchEgressNodes running with WatchEgressIP", func() {
		...
		ginkgo.It("should only get assigned EgressIPs which matches their subnet when the node is tagged", func() {
			app.Action = func(ctx *cli.Context) error {
				...
				gomega.Eventually(isEgressAssignableNode(node2.Name)).Should(gomega.BeFalse())
				gomega.Expect(getEgressIPAllocatorFirstConfigSafely(node1.Name).V4.Net).To(gomega.Equal(ip1V4Sub))
				gomega.Expect(getEgressIPAllocatorFirstConfigSafely(node1.Name).V6.Net).To(gomega.Equal(ip1V6Sub))
				gomega.Expect(getEgressIPAllocatorFirstConfigSafely(node2.Name).V4.Net).To(gomega.Equal(ip2V4Sub))
