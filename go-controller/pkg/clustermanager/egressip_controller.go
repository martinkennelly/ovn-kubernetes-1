...
func (eIPC *egressIPController) isAnyClusterNodeIP(ip net.IP) *egressNode {
...
func (eIPC *egressIPController) patchReplaceEgressIPStatus(name string, statusItems []egressipv1.EgressIPStatusItem) error {
...
func (eIPC *egressIPController) executeCloudPrivateIPConfigOps(egressIPName string, ops map[string]*cloudPrivateIPConfigOp) error {
	for egressIP, op := range ops {
		...
		if op.toAdd != "" && op.toDelete != "" {
			...
			if _, err := eIPC.kube.UpdateCloudPrivateIPConfig(cloudPrivateIPConfig); err != nil {
				...
				eIPC.recorder.Eventf(&eIPRef, kapi.EventTypeWarning, "CloudUpdateFailed", "egress IP: %s for object EgressIP: %s could not be updated, err: %v", egressIP, egressIPName, err)
			...
			if _, err := eIPC.kube.CreateCloudPrivateIPConfig(&cloudPrivateIPConfig); err != nil {
				...
				eIPC.recorder.Eventf(&eIPRef, kapi.EventTypeWarning, "CloudAssignmentFailed", "egress IP: %s for object EgressIP: %s could not be created, err: %v", egressIP, egressIPName, err)
			...
			if err := eIPC.kube.DeleteCloudPrivateIPConfig(cloudPrivateIPConfigName); err != nil {
				...
				eIPC.recorder.Eventf(&eIPRef, kapi.EventTypeWarning, "CloudDeletionFailed", "egress IP: %s for object EgressIP: %s could not be deleted, err: %v", egressIP, egressIPName, err)
...
func (eIPC *egressIPController) executeCloudPrivateIPConfigChange(egressIPName string, toAssign, toRemove []egressipv1.EgressIPStatusItem) error {
...
func newEgressIPController(ovnClient *util.OVNClusterManagerClientset, wf *factory.WatchFactory, recorder record.EventRecorder) *egressIPController {
	kube := &kube.KubeOVN{
...
func (eIPC *egressIPController) initRetryFramework() {
...
func (eIPC *egressIPController) newRetryFramework(objectType reflect.Type) *objretry.RetryFramework {
...
func (eIPC *egressIPController) Start() error {
	var err error
...
func (eIPC *egressIPController) WatchEgressNodes() (*factory.Handler, error) {
...
func (eIPC *egressIPController) WatchCloudPrivateIPConfig() (*factory.Handler, error) {
...
func (eIPC *egressIPController) WatchEgressIP() (*factory.Handler, error) {
	return eIPC.retryEgressIPs.WatchResource()
}

func (eIPC *egressIPController) Stop() {
	close(eIPC.stopChan)
	eIPC.wg.Wait()
...
func (h *egressIPClusterControllerEventHandler) AddResource(obj interface{}, fromRetryLoop bool) error {
	...
	case factory.EgressNodeType:
		node := obj.(*kapi.Node)
		if err = h.eIPC.setupNodeForEgress(node); err != nil {
			return err
		}
...
func (h *egressIPClusterControllerEventHandler) SyncFunc(objs []interface{}) error {
	...
	if h.syncFunc != nil {
		...
		switch h.objType {
		case factory.EgressNodeType:
			syncFunc = h.eIPC.initClusterEgressNodes
		case factory.EgressIPType,
...
func (eIPC *egressIPController) getSortedEgressData() ([]*egressNode, map[string]egressIPNodeStatus) {
...
func (eIPC *egressIPController) setupNodeForEgress(node *v1.Node) error {
...
func (eIPC *egressIPController) initClusterEgressNodes(nodes []interface{}) error {
	go eIPC.checkEgressNodesReachability()
	return nil
}

func (eIPC *egressIPController) setNodeEgressAssignable(nodeName string, isAssignable bool) {
	eIPC.allocator.Lock()
...
func (eIPC *egressIPController) isEgressNodeReady(egressNode *kapi.Node) bool {
...
func (eIPC *egressIPController) checkEgressNodesReachability() {
...
func checkEgressNodesReachabilityIterate(eIPC *egressIPController) {
...
func (eIPC *egressIPController) isReachable(nodeName string, mgmtIPs []net.IP, healthClient healthcheck.EgressIPHealthClient) bool {
	// Check if we need to do node reachability check
...
func (eIPC *egressIPController) isEgressNodeReachable(egressNode *kapi.Node) bool {
	eIPC.allocator.Lock()
	defer eIPC.allocator.Unlock()
...
func (eIPC *egressIPController) setNodeEgressReady(nodeName string, isReady bool) {
	eIPC.allocator.Lock()
	defer eIPC.allocator.Unlock()
	if eNode, exists := eIPC.allocator.cache[nodeName]; exists {
...
func (eIPC *egressIPController) setNodeEgressReachable(nodeName string, isReachable bool) {
	eIPC.allocator.Lock()
	defer eIPC.allocator.Unlock()
	if eNode, exists := eIPC.allocator.cache[nodeName]; exists {
		eNode.isReachable = isReachable
...
func (eIPC *egressIPController) addEgressNode(nodeName string) error {
	var errors []error
...
func (eIPC *egressIPController) deleteNodeForEgress(node *v1.Node) {
	eIPC.allocator.Lock()
...
func (eIPC *egressIPController) deleteEgressNode(nodeName string) error {
...
func (eIPC *egressIPController) initEgressIPAllocator(node *kapi.Node) (err error) {
	eIPC.allocator.Lock()
	defer eIPC.allocator.Unlock()
...
func (eIPC *egressIPController) deleteAllocatorEgressIPAssignments(statusAssignments []egressipv1.EgressIPStatusItem) {
	eIPC.allocator.Lock()
	defer eIPC.allocator.Unlock()
...
func (eIPC *egressIPController) deleteAllocatorEgressIPAssignmentIfExists(name, egressIP string) string {
	eIPC.allocator.Lock()
	defer eIPC.allocator.Unlock()
...
func (eIPC *egressIPController) addAllocatorEgressIPAssignments(name string, statusAssignments []egressipv1.EgressIPStatusItem) {
	eIPC.allocator.Lock()
	defer eIPC.allocator.Unlock()
	for _, status := range statusAssignments {
		if eNode, exists := eIPC.allocator.cache[status.Node]; exists {
...
func (eIPC *egressIPController) reconcileEgressIP(old, new *egressipv1.EgressIP) (err error) {
	...
	metrics.RecordEgressIPCount(getEgressIPAllocationTotalCount(eIPC.allocator))
	return nil
}
...
func (eIPC *egressIPController) assignEgressIPs(name string, egressIPs []string) []egressipv1.EgressIPStatusItem {
	eIPC.allocator.Lock()
	defer eIPC.allocator.Unlock()
	...
	if len(assignableNodes) == 0 {
		...
		eIPC.recorder.Eventf(&eIPRef, kapi.EventTypeWarning, "NoMatchingNodeFound", "no assignable nodes for EgressIP: %s, please tag at least one node with label: %s", name, util.GetNodeEgressLabel())
	...
	if len(assignments) == 0 {
		...
		eIPC.recorder.Eventf(&eIPRef, kapi.EventTypeWarning, "NoMatchingNodeFound", "No matching nodes found, which can host any of the egress IPs: %v for object EgressIP: %s", egressIPs, name)
	...
	if len(assignments) < len(egressIPs) {
		...
		eIPC.recorder.Eventf(&eIPRef, kapi.EventTypeWarning, "UnassignedRequest", "Not all egress IPs for EgressIP: %s could be assigned, please tag more nodes", name)
	}
	return assignments
}
...
// start going this.
func (eIPC *egressIPController) validateEgressIPStatus(name string, items []egressipv1.EgressIPStatusItem) (map[egressipv1.EgressIPStatusItem]string, map[egressipv1.EgressIPStatusItem]string) {
	eIPC.allocator.Lock()
	defer eIPC.allocator.Unlock()
...
func (eIPC *egressIPController) reconcileCloudPrivateIPConfig(old, new *ocpcloudnetworkapi.CloudPrivateIPConfig) error {
...
func (eIPC *egressIPController) removePendingOpsAndGetResyncs(egressIPName, egressIP string) ([]egressipv1.EgressIP, error) {
	eIPC.pendingCloudPrivateIPConfigsMutex.Lock()
	defer eIPC.pendingCloudPrivateIPConfigsMutex.Unlock()
	ops, pending := eIPC.pendingCloudPrivateIPConfigsOps[egressIPName]
	if !pending {
