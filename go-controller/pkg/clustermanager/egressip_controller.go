	...
	for _, egressIP := range egressIPs {
		...
		for _, eNode := range assignableNodes {
			...
			if isOVNManagedNetwork, err := util.IsOVNManagedNetwork(node, eIP); err != nil {
				...
				if err != nil {
					klog.Errorf("Failed to get non OVN managed networks for node %s: %v", node.Name, err)
					continue
				}
				if network == "" {
					continue
		...
		for i := 0; i < len(assignableNodes) && !assignmentSuccessful; i++ {
			...
			for _, egressIPConfig := range eNode.egressIPConfigs {
				if egressIPConfig.Capacity.IP < util.UnlimitedNodeCapacity {
					// TODO: fix me - int cap or node cap?
					if egressIPConfig.Capacity.IP-len(eNode.allocations) <= 0 {
						klog.V(5).Infof("Additional allocation on Node: %s exhausts it's IP capacity, trying another node", eNode.name)
						continue
					}
				}
				if egressIPConfig.Capacity.IPv4 < util.UnlimitedNodeCapacity && utilnet.IsIPv4(eIP) {
					//TODO fix me same as above
					if egressIPConfig.Capacity.IPv4-getIPFamilyAllocationCount(eNode.allocations, false) <= 0 {
						klog.V(5).Infof("Additional allocation on Node: %s exhausts it's IPv4 capacity, trying another node", eNode.name)
						continue
					}
				}
				if egressIPConfig.Capacity.IPv6 < util.UnlimitedNodeCapacity && utilnet.IsIPv6(eIP) {
					if egressIPConfig.Capacity.IPv6-getIPFamilyAllocationCount(eNode.allocations, true) <= 0 {
						klog.V(5).Infof("Additional allocation on Node: %s exhausts it's IPv6 capacity, trying another node", eNode.name)
						continue
					}
				}

				if (egressIPConfig.V6.Net != nil && egressIPConfig.V6.Net.Contains(eIP)) ||
					(egressIPConfig.V4.Net != nil && egressIPConfig.V4.Net.Contains(eIP)) {
					assignments = append(assignments, egressipv1.EgressIPStatusItem{
						Node:     eNode.name,
						EgressIP: eIP.String(),
...
func (eIPC *egressIPClusterController) isEgressIPAddrConflict(egressIP net.IP) (bool, error) {
	...
	for _, node := range nodes {
		...
		for _, nodeHostAddressT := range nodeHostAddressesSet.UnsortedList() {
			...
			ip, _, err := net.ParseCIDR(nodeHostAddress)
			if err != nil {
				klog.Errorf("Failed to parse host address %s. Unable to consider if this address is in conflict "+
					"with egress IP %s: %v", nodeHostAddress, egressIP.String(), err)
				continue
			}
			if ip == nil {
...
func (eIPC *egressIPClusterController) validateEgressIPStatus(name string, items []egressipv1.EgressIPStatusItem) (map[egressipv1.EgressIPStatusItem]string, map[egressipv1.EgressIPStatusItem]string) {
	...
	for _, eIPStatus := range items {
		...
		if !exists {
			...
			node, err := eIPC.watchFactory.GetNode(eNode.name)
			if err != nil {
				// node be not be present if its deleted
				klog.Warningf("Allocator error: failed to validate and will not consider node %s for egress IP %s: %v", eNode.name, name, err)
			}
			isOVNManaged, err := util.IsOVNManagedNetwork(node, ip)
			if err != nil {
				klog.Warningf("Allocator error: failed to determine if IP %s is part of an OVN managed network for egress IP %s: %v", ip.String(), name, err)
			}
			isSecondaryNetwork, err := util.IsNonOVNNetworkContainingIP(node, ip)
			if err != nil {
				klog.Warningf("Allocator error: failed to determine if IP %s is ")
			}
