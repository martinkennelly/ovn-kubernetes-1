	...
	ginkgo.Context("on startup repair", func() {
		ginkgo.It("should delete stale logical router policies and EgressService address set IPs", func() {
			app.Action = func(ctx *cli.Context) error {
				...
				_, err := fakeOVN.asfIPs.NewAddressSet(egresssvc.GetEgressServiceAddrSetDbIDs(controllerName), []net.IP{net.ParseIP("1.2.3.4"), net.ParseIP("10.128.1.5"), net.ParseIP("10.128.2.5")})
				...
				fakeOVN.asfIPs.ExpectAddressSetWithIPs(egresssvc.GetEgressServiceAddrSetDbIDs(controllerName), []string{"10.128.1.5", "10.128.2.5"})
				return nil
			}
	...
	ginkgo.Context("on egress service changes", func() {
		...
		ginkgotable.DescribeTable("should create/update/delete OVN configuration", func(interconnectEnabled bool) {
			app.Action = func(ctx *cli.Context) error {
				...
				gomega.Eventually(fakeOVN.nbClient).Should(libovsdbtest.HaveData(expectedDatabaseState))
				fakeOVN.asfIPs.ExpectAddressSetWithIPs(egresssvc.GetEgressServiceAddrSetDbIDs(controllerName), expectedEgressSvcAddrSet)
				...
				gomega.Eventually(fakeOVN.nbClient).Should(libovsdbtest.HaveData(expectedDatabaseState))
				fakeOVN.asfIPs.ExpectAddressSetWithIPs(egresssvc.GetEgressServiceAddrSetDbIDs(controllerName), []string{})
	...
	ginkgo.Context("on nodes changes", func() {
		ginkgotable.DescribeTable("should create/update/delete logical router policies and address sets", func(interconnectEnabled bool) {
			app.Action = func(ctx *cli.Context) error {
				...
				fakeOVN.asfIPs.EventuallyExpectAddressSetWithIPs(nodeIPsASdbIDs, []string{node2IPv4, node2IPv6})
				...
				gomega.Eventually(fakeOVN.nbClient).Should(libovsdbtest.HaveData(expectedDatabaseState))
				fakeOVN.asfIPs.EventuallyExpectEmptyAddressSetExist(nodeIPsASdbIDs)
				fakeOVN.asfIPs.ExpectAddressSetWithIPs(egresssvc.GetEgressServiceAddrSetDbIDs(controllerName), []string{})
				return nil
			}
			err := app.Run([]string{app.Name})
