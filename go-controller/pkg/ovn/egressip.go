...
func (e *EgressIPController) isPodScheduledinLocalZone(pod *kapi.Pod) bool {
	...
	klog.Infof("## martin isPodScheduledinLocalZone() pod %s with node %s is local? %v", pod.Name, pod.Spec.NodeName, isLocalZonePod)
...
func (e *EgressIPController) isLocalZoneNode(node *kapi.Node) bool {
	...
	if e.zone == types.OvnDefaultZone {
		klog.Infof("## martin isLocalZoneNode() node %s is local", node.Name)
		return !util.HasNodeMigratedZone(node)
	}
	/** HACK END **/
	klog.Infof("## martin isLocalZoneNode() node %s is local? %v", node.Name, util.GetNodeZone(node) == e.zone)
