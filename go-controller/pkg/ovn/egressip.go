...
func (oc *DefaultNetworkController) addPodEgressIPAssignments(name string, statusAssignments []egressipv1.EgressIPStatusItem, pod *kapi.Pod) error {
	...
	for _, status := range statusAssignments {
		isLocalZoneNode, loadedEgressNode := oc.eIPC.nodeZoneState.Load(status.Node)
...
func (oc *DefaultNetworkController) syncLocalNodeZonesCache() error {
	...
	for _, node := range nodes {
		...
		// The EgressNode updates should constantly update this cache as nodes get added, updated and removed
...
func (oc *DefaultNetworkController) generateCacheForEgressIP() (map[string]egressIPCacheEntry, error) {
	...
	for _, egressIP := range egressIPs {
		...
		isAtLeastOneEgressNodeLocal := false
		for _, status := range egressIP.Status.Items {
			var nextHopIP string
			isEgressIPv6 := utilnet.IsIPv6String(status.EgressIP)
			_, isLocalZoneNode := oc.localZoneNodes.Load(status.Node)
			if isLocalZoneNode {
				...
				nextHopIP, err = oc.eIPC.getTransitSwitchIP(status.Node)
				if err != nil {
			...
			if !isAtLeastOneEgressNodeLocal { // are any of these status nodes local to this zone?
				isAtLeastOneEgressNodeLocal = isLocalZoneNode
			}
		}
		...
		for _, namespace := range namespaces {
			...
			for _, pod := range pods {
				...
				if !isAtLeastOneEgressNodeLocal && !oc.isPodScheduledinLocalZone(pod) {
...
func (oc *DefaultNetworkController) addEgressNode(node *v1.Node) error {
	...
	if node, ok := oc.addEgressNodeFailed.Load(node.Name); ok {
...
func (e *egressIPZoneController) addPodEgressIPAssignment(egressIPName string, status egressipv1.EgressIPStatusItem, pod *kapi.Pod, podIPs []*net.IPNet) (err error) {
	...
	if loadedEgressNode && isLocalZoneNode.(bool) {
		...
		if config.OVNKubernetesFeature.EnableInterconnect && (loadedPodNode && !isLocalZonePod.(bool)) {
	...
	if loadedEgressNode && loadedPodNode && isLocalZonePod.(bool) {
...
func (e *egressIPZoneController) deletePodEgressIPAssignment(egressIPName string, status egressipv1.EgressIPStatusItem, pod *kapi.Pod, podIPs []*net.IPNet) (err error) {
	...
	isLocalZonePod, loadedPodNode := e.nodeZoneState.Load(pod.Spec.NodeName)
	var ops []ovsdb.Operation
	if !loadedPodNode || isLocalZonePod.(bool) { // node is deleted (we can't determine zone so we always try and nuke OR pod is local to zone)
	...
	isLocalZoneNode, loadedEgressNode := e.nodeZoneState.Load(status.Node)
	if loadedEgressNode && isLocalZoneNode.(bool) {
		if config.OVNKubernetesFeature.EnableInterconnect && (!loadedPodNode || !isLocalZonePod.(bool)) { // node is deleted (we can't determine zone so we always try and nuke OR pod is remote to zone)
...
func (e *egressIPZoneController) addExternalGWPodSNATOps(ops []ovsdb.Operation, podNamespace, podName string, status egressipv1.EgressIPStatusItem) ([]ovsdb.Operation, error) {
	if config.Gateway.DisableSNATMultipleGWs {
		...
		isLocalZonePod, loadedPodNode := e.nodeZoneState.Load(pod.Spec.NodeName)
		if (loadedPodNode && isLocalZonePod.(bool)) && pod.Spec.NodeName == status.Node && util.PodNeedsSNAT(pod) {
...
func (e *egressIPZoneController) getTransitSwitchIP(nodeName string) (string, error) {
	...
	transitSwitchIP, err := util.ParseNodeTransitSwitchPortAddrs(node)
	if err != nil {
