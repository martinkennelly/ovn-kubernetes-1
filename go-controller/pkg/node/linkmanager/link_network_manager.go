...
func (c *Controller) reconcile() {
	...
	for _, link := range links {
		...
		wantedAddresses, found := c.store[link.Attrs().Name]
		...
		for _, foundAddress := range foundAddresses {
			...
			if foundAddress.Label == GetAssignedAddressLabel(link.Attrs().Name) && !containsAddress(wantedAddresses, foundAddress) {
				if err := netlinkOps.AddrDel(link, &foundAddress); err != nil {
					klog.Errorf("Link Network Manager: failed to delete address %q from link %q",
						foundAddress.String(), link.Attrs().Name)
				} else {
...
func GetAssignedAddressLabel(linkName string) string {
	// label length is limited to 15 chars
	return fmt.Sprintf("%.12sovn", md5.Sum([]byte(linkName)))
}
