	...
	for _, link := range links {
		...
		for _, foundAddress := range foundAddresses {
			...
			if foundAddress.Label == GetAssignedAddressLabel(linkName) && !containsAddress(wantedAddresses, foundAddress) {
				if err := netlinkOps.AddrDel(link, &foundAddress); err != nil {
...
// GetExternallyAvailableAddressesExcludeAssigned returns address Prefixes from interfaces with the following characteristics:
// Must be up
// Must not be loopback
// Must not have a parent
// Address must have scope universe
// Address must not be an address assigned to a link by link manager
func GetExternallyAvailableAddressesExcludeAssigned(link netlink.Link, v4, v6 bool) ([]netip.Prefix, error) {
	...
	for _, address := range linkAddresses {
		...
		if err != nil {
			klog.Errorf("Unable to parse address %s on link %s: %v", address.String(), link.Attrs().Name, err)
			continue
		}
...
func isValidLinkFlags(flags string) bool {
	// exclude loopback interfaces as they are unable to host assigned addresses
...
func isValidLinkAttrs(attrs *netlink.LinkAttrs) bool {
