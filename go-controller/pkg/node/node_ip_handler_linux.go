...
func (c *addressManager) isValidNodeIP(addr net.IP) bool {
	...
	if config.OVNKubernetesFeature.EnableEgressIP && !util.PlatformTypeIsEgressIPCloudProvider() {
		...
		eipAddresses, err := c.getNonOVNEgressIPAddresses()
		if err != nil {
...
// getNonOVNEgressIPAddresses returns the set of egress IPs that are assigned to non-OVN managed interfaces. The
...
func (c *addressManager) getNonOVNEgressIPAddresses() (sets.Set[string], error) {
	node, err := c.watchFactory.GetNode(c.nodeName)
	if err != nil {
		return nil, fmt.Errorf("unable to get Node from informer: %v", err)
	}
	eipAddrs, err := util.ParseNodeEgressNonOVNAddrsAnnotation(node)
	if err != nil {
