...
var _ = ginkgo.Describe("IPTables Manager", func() {
	...
	ginkgo.Context("Ensure rules", func() {
		ruleArg := RuleArgs{[]string{"-s", "10.10.10.50", "-j", "MARK", "--set-mark", "1000"}}
		testRuleArgs := []RuleArgs{ruleArg,
			{
				[]string{"-s", "10.10.10.5", "-j", "MARK", "--set-mark", "2000"},
			},
		}
		differentRuleArg := RuleArgs{[]string{"-s", "10.10.10.50", "-j", "SNAT", "--to", "11.10.10.60"}}
