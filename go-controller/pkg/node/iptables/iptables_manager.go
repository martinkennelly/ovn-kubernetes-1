...
// Chain allows use to manage rules for a particular iptables table and chain for ipv4 and ipv6 proto
type Chain struct {
...
// RuleArg represents an iptables entry
type RuleArg struct {
	Args []string
}
			...
			for _, existingRuleArg := range existingRuleArgs {
				...
				if !found {
					if err = ipt.DeleteRule(existingChain.Table, existingChain.Chain, existingRuleArg.Args...); err != nil {
						klog.Errorf("IPTables manager: failed to delete stale rule (%s) in table %s and chain %s: %v",
