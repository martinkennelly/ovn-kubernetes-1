...
var _ = ginkgo.Describe("IPTables Manager", func() {
	...
	ginkgo.Context("Del rule", func() {
		ginkgo.It("doesn't fail when no rule to delete", func() {
			gomega.Expect(func() error {
				return testNS.Do(func(netNS ns.NetNS) error {
					return c.DeleteRule(*ruleWithDst)
				})
			}()).Should(gomega.Succeed())
		})
		...
		ginkgo.It("deletes a rule", func() {
			gomega.Expect(func() error {
				return testNS.Do(func(netNS ns.NetNS) error {
					if err := c.AddRule(*ruleWithDst); err != nil {
						return err
					}
					if err := c.DeleteRule(*ruleWithDst); err != nil {
						return err
					}
					return nil
				})
