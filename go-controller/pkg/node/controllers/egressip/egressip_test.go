...
import (
	...
	v12 "github.com/ovn-org/ovn-kubernetes/go-controller/pkg/crd/egressip/v1"
	fake2 "github.com/ovn-org/ovn-kubernetes/go-controller/pkg/crd/egressip/v1/apis/clientset/versioned/fake"
...
func setupTestEnvironment(stopCh chan struct{}, wg *sync.WaitGroup, namespaces []v1.Namespace, pods []v1.Pod, egressIPs []v12.EgressIP, node node) (ns.NetNS, *Controller, *util.OVNNodeClientset, cleanupFn, error) {
	...
	kubeClient := fake.NewSimpleClientset(&v1.NodeList{Items: []v1.Node{getNodeObj(node.name, node.annots, node.labels)}},
		&v1.NamespaceList{Items: namespaces}, &v1.PodList{Items: pods})
	egressIPClient := fake2.NewSimpleClientset(&v12.EgressIPList{Items: egressIPs})
...
var _ = table.FDescribeTable("EgressIP selectors",
	func(egressIPSelecteds []egressIPSelected, pods []v1.Pod, namespaces []v1.Namespace, node node) {
		...
		var err error
		egressIPList := make([]v12.EgressIP, 0)
		...
		for _, resource := range []struct {
			...
			klog.Errorf("## waiting for resource %s", resource.name)
		...
		gomega.Eventually(func() error {
			var foundIPTRules []iptables.RuleArgs
			err := testNS.Do(func(netNS ns.NetNS) error {
				c.triggerReconcile()
		...
		//ginkgo.By("Removing EgressIPs and ensuring state is restored")
		//
		//eIP := newEgressIP(egressIP2Name, egressIP2IP, dummy2IPv4CIDR, node1Name, namespace1Label, egressPodLabel)
		//gomega.Eventually(testNS.Do(func(netNS ns.NetNS) error {
		//	_, err := ovnNodeClient.KubeClient.CoreV1().Nodes().Get(context.TODO(), node1Name, )
		//
		//	return err
		...
		//	gomega.Expect(err).ShouldNot(gomega.HaveOccurred())
		//	if len(egressIPs) != 2 {
		//		return fmt.Errorf("failed to find two eip but found %d", len(egressIPs))
		//	}
		//	return nil
		//})).WithTimeout(oneSec).Should(gomega.Succeed())
		////klog.Errorf("############# egress IPs deleted and reconciled")
		////gomega.Eventually(func() error {
		////	return testNS.Do(func(netNS ns.NetNS) error {
		////		if err := c.reconcile(); err != nil {
		////			return err
		////		}
		////		return checkForCleanSlate(testNS, c, node)
		////	})
		////}).WithTimeout(oneSec).Should(gomega.Succeed())
	...
	table.Entry("configures nothing when EIPs dont select anything",
		...
		[]v1.Pod{newPodWithLabels(namespace1, pod1Name, node1Name, pod1IPv4, map[string]string{})},
		[]v1.Namespace{newNamespaceWithLabels(namespace1, namespace1Label)},
		node{
			node1Name,
			nodeLabels,
			nodeAnnotations,
	...
	table.Entry("configures one EIP and one Pod",
		...
		[]v1.Pod{newPodWithLabels(namespace1, pod1Name, node1Name, pod1IPv4, egressPodLabel)},
		[]v1.Namespace{newNamespaceWithLabels(namespace1, namespace1Label)},
		node{
			node1Name,
			nodeLabels,
	...
	table.Entry("configures one EIP and multiple pods",
		...
		[]v1.Namespace{newNamespaceWithLabels(namespace1, namespace1Label), newNamespaceWithLabels(namespace2, map[string]string{})},
		node{
			node1Name,
			nodeLabels,
			nodeAnnotations,
	...
	table.Entry("configures one EIP and multiple namespaces and multiple pods",
		...
		[]v1.Namespace{newNamespaceWithLabels(namespace1, namespace1Label), newNamespaceWithLabels(namespace2, namespace1Label),
...
var _ = ginkgo.Describe("Reconcile", func() {
	...
	ginkgo.BeforeEach(func() {
		...
		netNS, c, ovnClient, cleanupFn, err = setupTestEnvironment(stopCh, wg, []v1.Namespace{ns}, []v1.Pod{pod}, []v12.EgressIP{
...
func newEgressIP(name, ip, network, node string, namespaceLabels, podLabels map[string]string) v12.EgressIP {
	...
	return v12.EgressIP{
		...
		Status: v12.EgressIPStatus{
			Items: []v12.EgressIPStatusItem{{
				node,
				ip,
				ipnet.String(),
			}},
...
func validateChainJump(ipt *iptables.Controller, chain utiliptables.Chain, jumpChain string) (bool, error) {
	...
	for _, ruleArg := range ruleArgs {
		klog.Info("## checking if seen rule args %v is equal %v", ruleArg.Args, expectedArgs)
...
func getPod(pods []v1.Pod, name string) v1.Pod {
