...
func (c *Controller) reconcile() error {
	...
	for _, link := range links {
		...
		// this link will be routable and therefore not suitable for EIP assignment.
		for _, address := range addresses {
			netMaskSize, _ := address.Mask.Size()
			if address.IP.To4() != nil && netMaskSize == 32 {
				continue
			}
			if address.IP.To4() == nil && netMaskSize == 128 {
				continue
			}
	...
	c.removeStaleAddresses(linkAddresses, validIPAAddresses)
