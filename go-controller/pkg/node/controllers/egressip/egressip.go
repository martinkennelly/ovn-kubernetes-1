...
func (c *Controller) Run(stopCh <-chan struct{}, wg *sync.WaitGroup) error {
	...
	if err := c.ruleManager.OwnPriority(routeRulePriority); err != nil {
		klog.Errorf("Egress IP controller: failed to own priority %d - stale entries maybe present: %v", routeRulePriority, err)
	}
	if c.v4 {
...
func (c *Controller) reconcile() error {
	...
	validRoutingRules := make([]netlink.Rule, 0)
	...
	for _, eIP := range eIPs {
		...
		for _, status := range eIP.Status.Items {
			...
			routingRules, err := generateRoutingRules(selectedPods, link.Attrs().Index, c.v4, c.v6)
			if err != nil {
				klog.Errorf("Failed to generate a list of routing rules for Egress IP %s: %v", eIP.Name, err)
				continue
			}
			validRoutingRules = append(validRoutingRules, routingRules...)
			v4IPTablesRules, v6IPTablesRules, err := generateIPTablesSrcNATRules(selectedPods, status.EgressIP, link.Attrs().Name, c.v4, c.v6)
			if err != nil {
				klog.Errorf("Failed to generate a list of IPTable rules for Egress IP %s: %v", eIP.Name, err)
				continue
			}
			validV4IPTablesRules = append(validV4IPTablesRules, v4IPTablesRules...)
			validV6IPTablesRules = append(validV6IPTablesRules, v6IPTablesRules...)
	...
	c.removeStaleRoutingRules(existingRules, validRoutingRules)
	// ensure what we expect is present. It's safe to send multiple adds
	for _, validRule := range validRoutingRules {
...
func generateIPTablesSrcNATRules(pods []*corev1.Pod, eIPIP, infName string, v4, v6 bool) ([]iptables.RuleArg, []iptables.RuleArg, error) {
	validV4IPTablesRules := make([]iptables.RuleArg, 0)
	validV6IPTablesRules := make([]iptables.RuleArg, 0)
	for _, pod := range pods {
		for _, podIP := range pod.Status.PodIPs {
			podIPNet, isPodIPv6, err := getIPTablesSrcFromIP(podIP.IP)
			if err != nil {
				return nil, nil, fmt.Errorf("unable to get pod %s/%s network information to generate routing rules: %v",
					pod.Namespace, pod.Name, err)
			}
			args := []string{"-s", podIPNet.String(), "-o", infName, "-j", "SNAT", "--to-source", eIPIP}
	...
	return validV4IPTablesRules, validV6IPTablesRules, nil
}

func generateRoutingRules(selectedPods []*corev1.Pod, infIndex int, v4, v6 bool) ([]netlink.Rule, error) {
	validRules := make([]netlink.Rule, 0)
	for _, pod := range selectedPods {
		r := netlink.NewRule()
		r.Table = getRouteTableID(infIndex)
		r.Priority = routeRulePriority
		for _, podIP := range pod.Status.PodIPs {
			podIPNet, isPodIPv6, err := getIPTablesSrcFromIP(podIP.IP)
			if err != nil {
				return nil, fmt.Errorf("unable to get pod %s/%s network information to generate routing rules: %v",
					pod.Namespace, pod.Name, err)
			}
			r.Src = podIPNet
			if isPodIPv6 && v6 {
...
func getIPTablesSrcFromIP(ipStr string) (*net.IPNet, bool, error) {
	var err error
	var ip net.IP
	var ipNet *net.IPNet
	var v6 bool
	...
	if utilnet.IsIPv6(ip) {
		v6 = true
		_, ipNet, err = net.ParseCIDR(fmt.Sprintf("%s/128", ipStr))
	} else {
		_, ipNet, err = net.ParseCIDR(fmt.Sprintf("%s/32", ipStr))
	}
	return ipNet, v6, err
}
