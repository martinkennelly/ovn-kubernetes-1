...
func (c *Controller) reconcile() error {
	...
	for _, eIP := range eIPs {
		...
		for _, status := range eIP.Status.Items {
			...
			routingRules, err := generateRoutingRules(selectedPods, link.Attrs().Index)
			if err != nil {
			...
			v4IPTablesRules, v6IPTablesRules, err := generateIPTablesSrcNATRules(selectedPods, status.EgressIP, link.Attrs().Name)
			if err != nil {
...
func generateIPTablesSrcNATRules(pods []*corev1.Pod, eIPIP, infName string) ([]iptables.RuleArg, []iptables.RuleArg, error) {
	validV4IPTablesRules := make([]iptables.RuleArg, 0)
	validV6IPTablesRules := make([]iptables.RuleArg, 0)
	...
	for _, pod := range pods {
		...
		if err != nil {
			return nil, nil, fmt.Errorf("failed to generate iptables src NAT rules for pod %s/%s: %v", pod.Namespace, pod.Name, err)
		}
		for _, ip := range ips {
			if len(ip) == 0 {
				return nil, nil, fmt.Errorf("invalid IP %q for pod %s/%s", ip.String(), pod.Namespace, pod.Name)
			}
			if ip.To4() != nil { // v4
				ipFullMask := fmt.Sprintf("%s/32", ip.String())
				validV4IPTablesRules = append(validV4IPTablesRules, iptables.RuleArg{Args: []string{"-s", ipFullMask,
					"-o", infName, "-j", "SNAT", "--to-source", eIPIP}})
			} else { // v6
				ipFullMask := fmt.Sprintf("%s/128", ip.String())
				validV6IPTablesRules = append(validV6IPTablesRules, iptables.RuleArg{Args: []string{"-s", ipFullMask,
					"-o", infName, "-j", "SNAT", "--to-source", eIPIP}})
			}
		}
	}
...
func generateRoutingRules(selectedPods []*corev1.Pod, infIndex int) ([]netlink.Rule, error) {
