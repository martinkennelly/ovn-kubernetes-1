...
type Controller struct {
	mu    *sync.Mutex
	rules []ipRule
	// only explicit IP rules (via fn AddIPRule) are allowed when a priority is owned. Other IP rules will be removed.
...
// AddIPRule ensures an IP rule is applied even if it is altered by something else, it will be restored
func (rm *Controller) AddIPRule(rule netlink.Rule) error {
	rm.mu.Lock()
	defer rm.mu.Unlock()
...
// DeleteIPRule stops managed an IP rule and ensures its deleted
func (rm *Controller) DeleteIPRule(rule netlink.Rule) error {
	rm.mu.Lock()
	defer rm.mu.Unlock()
