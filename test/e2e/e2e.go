...
func isolateIPv6Networks(networkA, networkB string) error {
	if provider.Get().GetName() != provider.KinD {
		return fmt.Errorf("unsupported")
	}
	if containerruntime.GetContainerRuntime() != "docker" {
...
var _ = ginkgo.Describe("e2e control plane", func() {
	...
	var (
		...
		infraProvider         provider.Context
	)

	ginkgo.BeforeEach(func() {
		infraProvider = provider.Get().NewTestContext()
		...
		controlPlanePods, err := f.ClientSet.CoreV1().Pods(provider.GetOVNKubernetesNamespace()).List(context.Background(), metav1.ListOptions{
	...
	ginkgo.AfterEach(func() {
		err := infraProvider.CleanUp()
		framework.ExpectNoError(err, "failed to cleanup deployment configuration")
	})
	...
	ginkgo.It("should provide Internet connection continuously when ovnkube-node pod is killed", func() {
		...
		err = restartOVNKubeNodePod(f.ClientSet, provider.GetOVNKubernetesNamespace(), nodeName)
		framework.ExpectNoError(err)
	...
	ginkgo.It("should provide Internet connection continuously when pod running master instance of ovnkube-control-plane is killed", func() {
		...
		ovnKubeControlPlaneNode, err := findOvnKubeControlPlaneNode(provider.GetOVNKubernetesNamespace(), controlPlanePodName, controlPlaneLeaseName)
		...
		ovnKubeNamespace := provider.GetOVNKubernetesNamespace()
	...
	ginkgo.It("should provide Internet connection continuously when all ovnkube-control-plane pods are killed", func() {
		...
		for _, pod := range podList.Items {
			if strings.HasPrefix(pod.Name, controlPlanePodName) && !strings.HasPrefix(pod.Name, "ovs-node") {
				framework.Logf("%q", pod.Namespace)
				err = e2epod.DeletePodWithWaitByName(context.TODO(), f.ClientSet, pod.Name, provider.GetOVNKubernetesNamespace())
				framework.ExpectNoError(err, fmt.Sprintf("failed to delete pod %s", pod.Name))
				framework.Logf("Deleted control plane pod %q", pod.Name)
			}
		}
	...
	ginkgo.Describe("test node readiness according to its defaults interface MTU size", func() {
		...
		ginkgo.BeforeEach(func() {
			...
			res, err := infraProvider.ExecK8NodeCommand(testNodeName, []string{"cat", fmt.Sprintf("/sys/class/net/%s/mtu", provider.GetExternalBridgeName())})
			if err != nil {
		...
		ginkgo.AfterEach(func() {
			...
			_, err := infraProvider.ExecK8NodeCommand(testNodeName, []string{"ip", "link", "set", provider.GetExternalBridgeName(), "mtu", fmt.Sprintf("%d", originalMTU)})
			if err != nil {
				framework.Failf("could not reset MTU of interface: %s", err)
			}

			// restart ovnkube-node pod
			if err := restartOVNKubeNodePod(f.ClientSet, provider.GetOVNKubernetesNamespace(), testNodeName); err != nil {
		...
		ginkgo.It("should get node not ready with a too small MTU", func() {
			...
			_, err := infraProvider.ExecK8NodeCommand(testNodeName, []string{"ip", "link", "set", provider.GetExternalBridgeName(), "mtu", "1000"})
			if err != nil {
		...
		ginkgo.It("should get node ready with a big enough MTU", func() {
			...
			_, err := infraProvider.ExecK8NodeCommand(testNodeName, []string{"ip", "link", "set", provider.GetExternalBridgeName(), "mtu", "2000"})
			if err != nil {
				framework.Failf("could not set MTU of interface: %s", err)
			}
...
var _ = ginkgo.Describe("test e2e pod connectivity to host addresses", func() {
	...
	f := wrappedTestFramework(svcname)
	infraCtx := provider.Get().NewTestContext()
	...
	ginkgo.BeforeEach(func() {
		...
		_, err = infraCtx.ExecK8NodeCommand(workerNodeName, []string{"ip", "a", "add",
			fmt.Sprintf("%s/%s", targetIP, singleIPMask), "dev", provider.GetExternalBridgeName()})
		framework.ExpectNoError(err, "failed to add IP to %s", workerNodeName)
	})

	ginkgo.AfterEach(func() {
		_, err := infraCtx.ExecK8NodeCommand(workerNodeName, []string{"ip", "a", "del",
			fmt.Sprintf("%s/%s", targetIP, singleIPMask), "dev", provider.GetExternalBridgeName()})
...
var _ = ginkgo.Describe("e2e ingress traffic validation", func() {
	...
	testDeploymentCtx := provider.Get().NewTestContext()
	...
	ginkgo.Context("Validating ingress traffic", func() {
		ginkgo.BeforeEach(func() {
			...
			port := provider.Get().GetContainerPort()
			externalContainer = provider.ExternalContainer{Name: clientContainerName, Image: agnhostImage, Network: provider.Network{Type: provider.Primary},
				ContainerPort: port, CMD: []string{"netexec", fmt.Sprintf("--http-port=%d", port)}}
			externalContainer, err = testDeploymentCtx.CreateExternalContainer(externalContainer)
			framework.ExpectNoError(err, "failed to create external service", externalContainer.String())
		})

		ginkgo.AfterEach(func() {
			err := testDeploymentCtx.DeleteExternalContainer(externalContainer)
			framework.ExpectNoError(err, "failed to clean up test deployment context")
		})
				...
				for _, node := range nodes.Items {
					for _, nodeAddress := range node.Status.Addresses {
						...
						for i := 0; i < maxTries; i++ {
							epHostname := pokeEndpointViaExternalService(testDeploymentCtx, externalContainer, protocol, nodeAddress.Address, nodePort, "hostname")
				...
				gomega.Eventually(func() (r bool) {
					...
					for _, ipAddresses := range ipv4Addresses {
						for _, targetHost := range ipAddresses {
							...
							res, err := testDeploymentCtx.ExecExternalContainerCommand(externalContainer, curlCommand)
							if err != nil {
					...
					for nodeName, ipAddresses := range nodeToAddressesMapping {
						for _, address := range ipAddresses {
							...
							for _, protocol := range []string{"http", "udp"} {
								...
								valid := false
								for i := 0; i < maxTries; i++ {
									epHostname := pokeEndpointViaExternalService(testDeploymentCtx, externalContainer, protocol, address, port, "hostname")
									responses.Insert(epHostname)
			...
			for _, protocol := range []string{"http", "udp"} {
				for _, node := range nodes.Items {
					for _, nodeAddress := range node.Status.Addresses {
						...
						for i := 0; i < maxTries; i++ {
							epHostname := pokeEndpointViaExternalService(testDeploymentCtx, externalContainer, protocol, nodeAddress.Address, nodePort, "hostname")
							epClientIP := pokeEndpointViaExternalService(testDeploymentCtx, externalContainer, protocol, nodeAddress.Address, nodePort, "clientip")
		...
		ginkgo.It("Should be allowed by externalip services", func() {
			...
			for _, externalAddress := range addresses {
				...
				valid := isNeighborEntryStable(testDeploymentCtx, externalContainer, externalAddress, 10)
				...
				for _, protocol := range []string{"http", "udp"} {
					...
					ginkgo.By(
						fmt.Sprintf("Hitting the external service on IP %s, protocol %s, port %d and reaching all the endpoints",
							...
							protocol,
							externalPort))
					valid = pokeExternalIpService(testDeploymentCtx, externalContainer, protocol, externalAddress, externalPort, maxTries, nodesHostnames)
	...
	ginkgo.Context("Validating ingress traffic to manually added node IPs", func() {
		ginkgo.BeforeEach(func() {
			...
			err = testDeploymentCtx.AttachSecondaryNetworkExternalContainer(provider.Network{Type: provider.Secondary, Name: "kindexgw"}, externalContainer)
			...
			for i, node := range nodes.Items {
				...
				_, err := testDeploymentCtx.ExecK8NodeCommand(node.Name, []string{"ip", "addr", "add", newIP, "dev", provider.GetExternalBridgeName()})
				if err != nil {
					framework.Failf("failed to add new Addresses to node %s: %v", node.Name, err)
				}
				testDeploymentCtx.AddCleanUpFn(func() error {
					_, err := testDeploymentCtx.ExecK8NodeCommand(node.Name, []string{"ip", "addr", "del", newIP, "dev", provider.GetExternalBridgeName()})
					if err != nil {
						framework.Failf("failed to add new Addresses to node %s: %v", node.Name, err)
					}
					return nil
		...
		ginkgo.It("Should be allowed by externalip services to a new node ip", func() {
			...
			for _, protocol := range []string{"http", "udp"} {
				for _, externalAddress := range newNodeAddresses {
					...
					for i := 0; i < maxTries; i++ {
						epHostname := pokeEndpointViaExternalService(testDeploymentCtx, externalContainer, protocol, externalAddress, externalPort, "hostname")
						responses.Insert(epHostname)
...
var _ = ginkgo.Describe("e2e ingress to host-networked pods traffic validation", func() {
	...
	ginkgo.Context("Validating ingress traffic to Host Networked pods with externalTrafficPolicy=local", func() {
		ginkgo.BeforeEach(func() {
			...
			port := provider.Get().GetContainerPort()
			externalContainer = provider.ExternalContainer{Name: clientContainerName, Image: agnhostImage, Network: provider.Network{Type: provider.Primary},
				CMD: []string{"netexec", fmt.Sprintf("--http-port=%d", port)}, ContainerPort: port}
			externalContainer, err = testDeploymentCtx.CreateExternalContainer(externalContainer)
		})

		ginkgo.AfterEach(func() {
			err := testDeploymentCtx.CleanUp()
			framework.ExpectNoError(err, "failed to clean up deployment specific resources")
...
var _ = ginkgo.Describe("e2e br-int flow monitoring export validation", func() {
	...
	ginkgo.AfterEach(func() {
		...
		err := testDeploymentCtx.CleanUp()
		framework.ExpectNoError(err, "failed to cleanup resources created")
	})
	...
	ginkgo.DescribeTable("Should validate flow data of br-int is sent to an external gateway",
		func(protocol flowMonitoringProtocol, collectorPort uint16) {
			...
			externalContainer := provider.ExternalContainer{Name: collectorContainer, Image: "cloudflare/goflow",
				Network: provider.Network{Type: provider.Primary}, CMD: []string{"-kafka=false"}}
			var err error
			externalContainer, err = testDeploymentCtx.CreateExternalContainer(externalContainer)
			if err != nil {
			...
			collectorContainerLogsTest := func() wait.ConditionFunc {
				return func() (bool, error) {
					collectorContainerLogs, err := testDeploymentCtx.GetExternalContainerLogs(externalContainer)
					if err != nil {
...
var _ = ginkgo.Describe("e2e delete databases", func() {
	...
	fileExistsOnPod := func(f *framework.Framework, namespace string, pod *v1.Pod, file string) bool {
		...
		_, err := e2ekubectl.RunKubectl(ovnKubeNamespace, "exec", pod.Name, containerFlag, "--", "ls", file)
		if err == nil {
			return true
		}
